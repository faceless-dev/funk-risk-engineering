...
export function CaseTable({ cases, activeTab }: CaseTableProps) {
      ...
      {filteredCases.map((caseItem) => (
        ...
        >
          <div className="col-span-3">
            <Link href={`/cases/${caseItem.id}`} className="font-medium text-blue-600 hover:underline">
              {caseItem.id}: {caseItem.title}
            </Link>
          </div>
          <div className="col-span-2 truncate">
            <Link
              href={`/customers/${caseItem.customer.toLowerCase().replace(/\s+/g, "-")}`}
          ...
          <div className="col-span-2 truncate">
            <Link
              href={`/customers/${caseItem.customer.toLowerCase().replace(/\s+/g, "-")}/locations`}
